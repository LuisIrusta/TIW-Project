CREATE DATABASE IF NOT EXISTS TIW_DB;
USE TIW_DB;
...
CREATE TABLE projects (
    ...
    CONSTRAINT chk_proj_duration CHECK (duration_months BETWEEN 1 AND 240),
...
BEGIN
    ...
    DECLARE is_mgr   TINYINT;
    SELECT administrative INTO is_admin FROM users WHERE id = NEW.administrator_id;
    SELECT technical       INTO is_mgr   FROM users WHERE id = NEW.manager_id;
    IF is_admin IS NULL OR is_admin = 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'administrator must exist and have administrative=1';
    END IF;
    IF is_mgr IS NULL OR is_mgr = 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'manager must exist and have technical=1';
    END IF;
...
BEGIN
    ...
    -- ← MODIFICA: usa collaborator=1 invece di role='technical'
    SELECT collaborator INTO is_collab FROM users WHERE id = NEW.collaborator_id;
    IF is_collab IS NULL OR is_collab = 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'collaborator must exist and have collaborator=1';
    END IF;
